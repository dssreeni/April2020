...
titanic_train = pd.read_csv("titanic_train_alt.csv")
titanic_train.shape
titanic_train.info()
...
mean_cv_score = model_selection.cross_val_score(nb_estimator, X_train, y_train, cv=9).mean()
...
titanic_test.to_csv('submission_MultiNB.csv', columns=['PassengerId','Survived'],index=False)
...
titanic_test.to_csv('submission_MultiNB_Prob.csv', columns=['PassengerId','Survived'],index=False)
