...
sns.countplot(combined_data['MSZoning'], data=combined_data) #To get the most frequent value which is Mode
...
print(type(built_year_data['GarageYrBlt']))
...
#Finish like of Advanced EDA and Creatuive FE
...
param_grid = {'max_depth':[20, 25, 30], 'min_samples_split':[2, 4, 6]}
...
res = model_selection.cross_val_score(dt, train_X, train_Y, cv=10, scoring=metrics.make_scorer(rmse)).mean()

dt_grid = model_selection.GridSearchCV(dt, param_grid, cv=10, n_jobs=8)
dt_grid.fit(train_X,train_Y) #Builds a decision tree classifier from the training set (X, y).

#Need some syntax correction
X_test = test_data_new
log_sales_price = dt.predict(X_test)
#convert log values back to orignal values by using exponential loss
#house_test['SalePrice'] = np.exp(log_sales_price)
test_data['SalePrice'] = log_sales_price
test_data.to_csv("submission_House.csv", columns=['Id','SalePrice'], index=False)
